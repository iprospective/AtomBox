...
(function (ABX) {
  ...
  Cd("etat.journal", () => md("**D152** — journal par **domaine** et par **sévérité** dans `logs/`, un fichier par domaine plus `erreurs.log` (WARNING et plus, tous domaines) ; niveau réglable par domaine. Lu ici par `GET /etat/journal` : le produit **se laisse superviser**, il ne supervise pas."));
  Cd("regles", () => md("**D074** — le moteur remplace Sieve. Ce que Sieve ne fait pas : **compter les déclenchements** (**D075** § 1). Le chapitre 14 a trouvé ainsi un `allof` saisi à la place d'un `anyof`, muet depuis des années. Les règles existantes sont reprises en V1 (**D147**)."));
