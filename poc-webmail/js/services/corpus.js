...
(function (ABX) {
  ...
  function fabriqueMessages(fid, label, n, opt) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const m = {
        id: fid + "/m" + i, dossier_origine: fid,
        sens: sortant ? "out" : "in",
        from_nom: sortant ? Fx.MOI.nom : qui,
        from_adresse: sortant ? boite : adr,
        destinataires:   sortant ? [adr]  : [boite],
        boite,                                    // la boîte qui a reçu ou émis — Q026
        sujet: sujet(axe, label),
        corps: P.pick(Fx.CORPS),
      ...
      if (m.connu && P.with("spoof/" + m.id, () => P.next()) < .01) {
        m.usurpation = true;
        m.connu = false;
        m.from_adresse = ABX.Fmt.slug(qui).replace(/-/g, ".") + "@" +
      ...
      if (m.nature !== "humain") {
        ...
        const d = m.from_adresse.split("@")[1];
        m.connu  = false;
        m.from_nom   = m.nature === "liste" ? label : label + " — notifications";
        m.from_adresse   = (m.nature === "liste" ? "news@" : "noreply@") + d;
        if (m.nature === "liste") {
      ...
      m.reponse_possible = m.nature === "humain" ? "oui" : "avertir";
      if (m.nature !== "humain" &&
          P.with("dsn/" + m.id, () => P.next()) < .3) {
        m.reponse_possible = "non";
      ...
      if (m.reponse_possible !== "oui" && Fx.estAxe(axe) &&
          axe !== "notification" && axe !== "social")
        m.contact_alternatif = { nom: label, from_adresse: "contact@" + m.from_adresse.split("@")[1] };
    ...
    return out.sort((a, b) => b.date_recue - a.date_recue);
  }
  ...
  function deriveAbonnements() {
    ...
    for (const m of tous) {
      ...
      const v = par[m.listeId] = par[m.listeId] ||
        { id: m.listeId, label: m.listeLbl, axe: "abonnement", last: 0, list_id: m.list_id };
      if (m.date_recue > v.last) v.last = m.date_recue;
    }
  ...
  const Corpus = {
    ...
    engendre() {
      ...
      Fx.AXES.forEach(a => Fx.valeurs[a.id].forEach(v => {
        ...
        v.last = dossiers[v.id].length ? dossiers[v.id][0].date_recue : 0;
      }));
    ...
    applique() {
      St.crees.forEach(m => { parId[m.id] = m;
        (dossiers[m.dossier_origine] = dossiers[m.dossier_origine] || []).unshift(m); });
    ...
    vue(folder) {
      ...
      if (folder.kind === "axe")
        return tous.filter(m => hors(m) && (m.dossier || m.dossier_origine).startsWith(folder.axe + ":"));
      return tous.filter(m => hors(m) && (m.dossier || m.dossier_origine) === folder.id);
    },

    TRIS: {
      date_desc: (x, y) => y.date_recue - x.date_recue,
      date_asc:  (x, y) => x.date_recue - y.date_recue,
      from_nom:      (x, y) => x.from_nom.localeCompare(y.from_nom),
      subj:      (x, y) => x.sujet.localeCompare(y.sujet),
      taille:      (x, y) => y.taille - x.taille,
    },
